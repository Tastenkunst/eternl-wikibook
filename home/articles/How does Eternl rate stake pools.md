# Relevant parameters and how they are taken into account
...
Let’s first have a look at the parameters displayed in the Eternl’s staking section for each pool:
...
Each pool’s score is initially set to zero if the pledge is not fulfilled or is below 50 ADA.
Pools are then normalized by dividing their pledge by the maximum pledge of that list.

### 3. Median Normalization and Extra Points Calculation
The desired number of pools is around 500. The 250th pool in the sorted list is used as the median.
Each pool’s score is adjusted to match the median pool’s score.
Additional points are awarded to pools starting from the first, with 250 - current rank points added to each.
...
Additional weight is added based on the Return on Stake (RoS) over the last 12 epochs.

### 6. Single vs. Multi-Pool Operators:
The best pool of a multi-pool setup has its score adjusted to be no higher than the lowest score among the top 48 single pool operators, ensuring it does not rank above 49. Subsequent pools in the multi-pool setup receive even lower scores based on the best-ranking pool in the setup.
